...
#    l = []
#    for key, clip in zip(merged_dict.keys(), merged_dict.items()):
#        print(key, clip[1].shape[1])
#        l.append(clip[1].shape[1])
#    print(sorted(l))
    shortest_clip = min(merged_dict.items(), key=lambda x: x[1].shape[1])
    shortest_pixel_width = shortest_clip[1].shape[1]
  
    print(shortest_pixel_width)
...
def get_random_samples(matrix, n_samples, crop_width):
    ...
    samples=[]
    acc_len=matrix.shape[1]-crop_width
    for i in range(n_samples):
        
        random_num=randint(0, acc_len)
        
        clipped_mat = matrix[:,random_num:random_num+crop_width]
        #print(clipped_mat.shape[1])
        samples.append(clipped_mat)
        
        
    
    
#    clipped_mat = matrix[:, (matrix.shape[1] % crop_width):]
##    if (clipped_mat.shape[1]==0):
##        print('error')
##        return "error"
#        
...
#    merged_samples = build_class_sample_dict(merged_dict, n_samples,
#                                                crop_width)
#    
#    if type(merged_samples)==int:
#        del merged_dict[merged_samples]
#        merged_samples = build_class_sample_dict(merged_dict, n_samples,
#                                                crop_width)
...
#    for key, _ in merged_samples.iteritems():
#        path = '/media/hdd1/genfyp/processed/'
#        filename = 'M{}.npz'.format(key)
#        outfile = path + filename
#        np.savez(outfile, *merged_samples[key])
#        
...
#    merge_samps = [f for f in npz_files if f.startswith('M')]
...
#    max_samples = len(merge_samps)
    ...
    norm_select_samps = np.random.choice(norm_samps, size=max_samples,
                                         replace=False)
#    merge_select_samps = np.random.choice(merge_samps, size=max_samples,
#                                        replace=False)
    ...
    # REFACTOR this code!
    test_size = 0.2
#    num_test_samples = int(len(merge_select_samps)*test_size )
    num_test_samples = int(len(dep_select_samps) * test_size)
    train_samples = []
#    for sample in merge_select_samps[:-num_test_samples]:
...
if __name__ == '__main__':
    ...
    train_samples, train_labels, test_samples, \
        test_labels = rand_samp_train_test_split('/media/hdd1/genfyp/processed/')
    ...
    save_to_bucket('../../processed/train_samples.npz', 'train_samples.npz')
    save_to_bucket('../../processed/train_labels.npz', 'train_labels.npz')
    save_to_bucket('../../processed/test_samples.npz', 'test_samples.npz')
    save_to_bucket('../../processed/test_labels.npz', 'test_labels.npz')
    """
